...
source_stmt
	: string
          ...
          '{' source_items '}'
          { cfg_lexer_pop_context(lexer); }     { CHECK_WARNING(!g_hash_table_lookup(configuration->sources, $1), @1,
	...
	: string '{'
	  {
	    ...
	    CHECK_ERROR(cfg_parser_parse(&filter_expr_parser, lexer, (gpointer *) &last_filter_expr, NULL), @1, NULL);
          ...
          '}'                               { CHECK_WARNING(!g_hash_table_lookup(configuration->filters, $1), @1, "duplicate definition");
...
parser_stmt
        : string '{'
          ...
          } '}'	                                { CHECK_WARNING(!g_hash_table_lookup(configuration->parsers, $1), @1,
                                                                      "duplicate definition");
...
rewrite_stmt
        : string '{'
          ...
          } '}'                                 { CHECK_WARNING(!g_hash_table_lookup(configuration->rewriters, $1), @1,
                                                  "duplicate definition");
...
dest_stmt
        : string
          ...
          '{' dest_items '}'
          { cfg_lexer_pop_context(lexer); }         {  CHECK_WARNING(!g_hash_table_lookup(configuration->destinations, $1), @1,
                                                                   "duplicate definition");
...
options_item
  ...
  | KW_CUSTOM_DOMAIN '(' LL_STRING ')' { configuration->custom_domain = g_strdup($3); free($3); }
	...
	| KW_LOG_IW_SIZE '(' positive_integer ')'	{ msg_error("Using a global log-iw-size() option was removed, please use a per-source log-iw-size()", NULL); }
	| KW_LOG_FETCH_LIMIT '(' positive_integer ')'	{ msg_error("Using a global log-fetch-limit() option was removed, please use a per-source log-fetch-limit()", NULL); }
	...
	| KW_DNS_CACHE_SIZE '(' positive_integer ')'	{ configuration->dns_cache_size = $3; }
	| KW_DNS_CACHE_EXPIRE '(' positive_integer ')'	{ configuration->dns_cache_expire = $3; }
...
nonnegative_integer
        : LL_NUMBER
          {
            CHECK_WARNING(($1 >= 0), @1, "It should not be negative");
          }
        ;
...
positive_integer
        : LL_NUMBER
          {
            CHECK_WARNING(($1 > 0), @1, "It should be positive");
          }
        ;
...
source_reader_option
	...
	| KW_LOG_MSG_SIZE '(' positive_integer ')'	{ last_reader_options->msg_size = $3; }
	| KW_LOG_FETCH_LIMIT '(' positive_integer ')'	{ last_reader_options->fetch_limit = $3; }
	| KW_PAD_SIZE '(' nonnegative_integer ')'		{ last_reader_options->padding = $3; }
